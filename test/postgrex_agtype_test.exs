defmodule PostgrexAgtypeTest do
  ...
  describe "query!/3" do
    ...
    test "combines edge result rows with combine option", %{conn: conn, graph_name: graph_name} do
      ...
      [e1, e2] = PostgrexAgtype.query!(conn, graph_name, "MATCH (a:Alpha), (b:Beta) CREATE (b)-[e1:Rel]->(a), (a)-[e2:Rel]->(b) RETURN [ID(e1), ID(e2)]")

      observed = PostgrexAgtype.query!(conn, graph_name, "MATCH ()-[e:Rel]->() RETURN e", combine: true)

      expected =
        Graph.new()
        |> Graph.add_edge(beta_id, alpha_id, label: %{"id" => e1, "label" => "Rel", "properties" => %{}})
        |> Graph.add_edge(alpha_id, beta_id, label: %{"id" => e2, "label" => "Rel", "properties" => %{}})
